...
export default function CheckInVisitors() {
  ...
  const GetAllVisits = async () => {
    try {
      ...
      const response = await apiClient.get(`/visits?page=${pageIndex}&pageSize=${10}&status="CheckedIn"&search=${searchTerm}&from=${fromDate}&to=${toDate}`)
  ...
  const handleCheckout = async (visitId: string) => {
    ...
    if(visitId){
        const selectedVisit = allVisits.find(visit => visit.id === visitId)
            if (!selectedVisit || !selectedVisit.barcodeUrl || !selectedVisit.tagNumber) {
                toast.error("Selected visit does not have a valid barcode.")
                return
            }
        const response = await apiClient.post(`/visits/checkout`, {
            barcode: selectedVisit.barcodeUrl,
            tagNumber: selectedVisit.tagNumber,
        })

        if (response.status === 200 || response.status === 201) {
            console.log("Visitor checked-out successfully:", response.data)
            toast.success("Visitor checked-out successfully!")
            // Update the selected visit and allVisits array
            const updatedAllVisits = allVisits.map((visit) =>
              visit.id === visitId? {...visit, status: "checked-out" } : visit
            )
            setAllVisits(updatedAllVisits)
        } else {
            console.error("Failed to check-out visitor:", response.statusText)
            toast.error("Failed to check-out visitor. Please try again later.")
        }
